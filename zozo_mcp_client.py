...
def _fix_windows_mojibake(text: str) -> str:
    ...
    if "10061" in text:
        return (
            "WinError 10061: 接続拒否 "
            "(ZOZO MCP が待ち受けていません。ZOZO パネルか女郎花の ZOZO用準備から起動)"
        )
    return text
...
def _format_exception(exc: BaseException) -> str:
    ...
    if isinstance(exc, urllib.error.URLError):
        ...
        if isinstance(reason, OSError):
            ...
            if winerr == 10061 or getattr(reason, "errno", None) in (111, 61):
                return (
                    "ZOZO MCP 接続拒否（ポートで待ち受けなし）。"
                    "ZOZO Contact Solver で MCP を開始するか、拡張読込後に ZOZO用準備を再実行してください。"
                )
    ...
    if isinstance(exc, OSError):
        ...
        if winerr == 10061 or getattr(exc, "errno", None) in (111, 61):
            return "ZOZO MCP 接続拒否 (WinError 10061 / 待ち受けなし)"
        ...
        reply = self._post(
            {
                ...
                "params": {
                    ...
                    "capabilities": {},
                    "clientInfo": {"name": "ominaeshi", "version": "0.2.0"},
                },
            },
            timeout=5.0,
        )
        if not reply or "error" in reply or not self.session_id:
            raise RuntimeError("ZOZO MCP の initialize が有効なセッションを返しませんでした。")
...
def _ensure_group(client: MCPClient, *, name: str, group_type: str) -> str:
    ...
    run (when Housei-owned groups already exist) and leaves Dynamics empty.
...
def _assert_simulatable(client: MCPClient, *, cloth_uuid: str, cloth_object: str) -> None:
    ...
    if cloth is None:
        raise RuntimeError(f"ZOZO MCP 設定後に服グループ {cloth_uuid} が見つかりません。")
    if cloth.get("object_type") != "SHELL":
        raise RuntimeError(
            f"服グループの種類が {cloth.get('object_type')!r} です（SHELL が必要）。"
        )
    assigned = {
        item.get("name")
    ...
    if cloth_object not in assigned:
        raise RuntimeError(
            f"服オブジェクト '{cloth_object}' が SHELL グループに入っていません。"
            "このままだと ZOZO はシミュレート対象なしと表示します。"
        )
...
def _connection_summary(client: MCPClient) -> str:
    ...
    try:
        ...
        return f"接続不明 ({type(exc).__name__})"
    ...
    if connected and running:
        base = f"ソルバ接続済み ({ctype}"
    elif connected:
        base = f"ソルバ接続・サーバ未起動 ({ctype}"
    else:
        base = f"ソルバ未接続 ({ctype}"
    if project:
    ...
    *,
    ...
    while True:
        ...
        if deformation.get("has_cache") and int(deformation.get("frame_count", 0)) > 0:
            return f"ボディ変形 {int(deformation['frame_count'])} フレームをキャプチャ"
        if time.monotonic() >= deadline:
            raise TimeoutError(
                f"ZOZO ボディ変形キャプチャが {timeout_seconds:.0f} 秒以内に終わりませんでした。"
            )
        time.sleep(0.25)
...
def _configure(config: dict) -> dict:
    ...
    try:
        ...
        return {
            "status": "success",
            "message": (
                "ZOZO グループに服とボディを割り当て済みです。"
                "確認のあと Transfer、Run Simulation を実行してください。"
            ),
